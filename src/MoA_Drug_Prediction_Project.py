...
EACH CELL HAS BEEN DIVIDED LIKE THE ORIGINAL WAY IT WAS PLEASE COPY PASTE THIS
 INTO THE MOA KAGGLE PROJECT TO THE ACCESS THE DATA
...
df_feature_importance = pd.DataFrame(m.feature_importances_, 
...
#print(train_features.columns.get_loc('g-157'))
...
#from iterstrat.ml_stratifiers import MultilabelStratifiedKFold as KFold
test = test_features.iloc[:,top_feats]


#%%
'''
This is interesting. I decreased the number of epochs and the validation loss has lowered. I believe this is due to reduced fitting.
WeightNormalization improve results by a very small margin.
Using Multilabel Stratified K Fold library as it was something we intended to use in the beginning.
Goal is now to tinker with optimizer to reduce overfitting/reduce loss
'''
import tensorflow as tf
...
for index1, index2 in fold.split(test, y.values):
    ...
    model = tf.keras.Sequential(
        ...
        [tf.keras.layers.Input(len(t.columns)),
         ...
         tfa.layers.WeightNormalization(tf.keras.layers.Dense(len(train_features.columns)*6, 
                                                              activation = 'relu')),
         ...
         #tf.keras.layers.BatchNormalization(),
         #tfa.layers.WeightNormalization(tf.keras.layers.Dense(len(train_features.columns)*2, 
         #                                                     activation = 'relu')),
         #tf.keras.layers.GaussianDropout(0.4),
         #tf.keras.layers.BatchNormalization(),
         #tfa.layers.WeightNormalization(tf.keras.layers.Dense(750, activation = 'tanh')),
    ...
    model.compile(   loss='binary_crossentropy',
                     optimizer = tf.keras.optimizers.RMSprop(), #BEST ONE
                  #tf.keras.optimizers.Nadam(), SECOND BEST ONE BY 0.001 
                  #tf.keras.optimizers.Ftrl(),
                  #tf.keras.optimizers.Adamax(),  
                  #tf.keras.optimizers.Adagrad(),
                  #tf.keras.optimizers.Adadelta(learning_rate=0.01, rho=0.95, epsilon=1e-07, name='Adadelta'),
                  #tf.keras.optimizers.Adam(), #THIRD BEST ONE
                     metrics=["accuracy"])

    model.fit(Xtrain, Ytrain, batch_size = 128, epochs = 35,
              callbacks = tf.keras.callbacks.ReduceLROnPlateau(
                            monitor='val_loss', factor=0.01, patience=3, verbose=0, mode='min',
                            min_delta=0.001, cooldown=0, min_lr=0),
              verbose = 2,validation_data=(xvalidation, yvalidation))
    #print(len(test_features.values[0, top_feats]))
    #break
    predict = model.predict( test_features.iloc[:,top_feats])#test_features)
    #print(predict)
